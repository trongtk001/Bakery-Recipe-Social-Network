    ...
    return (
        <div className="border-t dark:border-gray-600">
                                    ...
                                    if(mess.senderID === isLogin.id){
                                        return (
                                                <div ref={scrollRef} key={index} className="flex lg:items-center flex-row-reverse">
                                                    <div className="w-14 h-14 rounded-full relative flex-shrink-0">
                                                        <img src="https://source.unsplash.com/random/?bakery,bake,1" alt="" className="absolute h-full rounded-full w-full"/>
                                                    </div>
                                                    <div className="text-white py-2 px-3 rounded bg-pink-600 relative h-full lg:mr-5 mr-2 lg:ml-20">
                                                        <p className="leading-6">{mess.messageBody}<i className="uil-grin-tongue-wink"></i> </p>
                                        ...
                                        return (
                                            <div ref={scrollRef} key={index} className="flex lg:items-center">
                                                <div className="w-14 h-14 rounded-full relative flex-shrink-0">
                                                    <img src="https://source.unsplash.com/random/?bakery" alt="" className="absolute h-full rounded-full w-full"/>
                                                </div>
                            ...
                            <div className="border-t flex p-6 dark:border-gray-700">
                                <input onKeyDown={(e) => {if(e.key === "Enter") handleChat()}} refs={textInput} value={message} onChange={e=>setMessage(e.target.value)} cols="1" rows="1" placeholder="Your Message.." className="border-0 flex-1 h-10 min-h-0 resize-none min-w-0 shadow-none dark:bg-transparent"></input>
                                <div className="flex h-full space-x-2">
                                    <button  onClick={handleChat} type="submit" className="bg-pink-600 font-semibold px-6 py-2 rounded-md text-white">Send</button>
                                </div>
                            </div>
                        </div>
    )
