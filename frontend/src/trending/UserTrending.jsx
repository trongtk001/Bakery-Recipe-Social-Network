...
export default function UserTrending(post) {
    ...
    return (
        <div class="flex gap-4 mb-10  w-full overflow-x-auto" >
           {post.post.list && post.post.list?.map((item, index)=>{
                return ( 
                    <Link key={index} to={`user/${item.id}`}  class="inline-block min-w-max">
