                    ...
                    {dataList &&  dataList.map((item, index)=>{
                        ...
                        return (
                            <div key={index} className='aspect-square'>
                                <div
                                    class="bg-blue-400 max-w-full h-full w-full rounded-lg relative overflow-hidden shadow uk-transition-toggle">
                                    <Link to={`post/${item.id}`} uk-toggle>
                                        <img src={item.recipe.steps.length >= 1 && item.recipe.steps[0].image } class="w-full h-full absolute object-cover inset-0" onError={(i) => (i.target.src = `https://source.unsplash.com/random/?bakery,bake,${index}`)} />
                                    </Link>
                                    <div
                                        ...
                                        <a href="#" class="lg:flex flex-1 items-center hidden">
                                            <div class="bg-gradient-to-tr from-yellow-600 to-pink-600 p-1 rounded-full transform -rotate-2 hover:rotate-3 transition hover:scale-105 m-0.5 mr-2">
                                                <img src={item.member.avatar ? item.member.avatar : 'https://images.squarespace-cdn.com/content/v1/54b7b93ce4b0a3e130d5d232/1519987020970-8IQ7F6Z61LLBCX85A65S/icon.png?format=1000w'}
                                        ...
                                        <div class="flex space-x-2 flex-1 lg:flex-initial justify-around">
                                            <a href="#"> <i class="uil-heart"></i>{item.likes.length} Like </a>
                                        </div>
                                    </div>
                                    </div>
                                </div>
