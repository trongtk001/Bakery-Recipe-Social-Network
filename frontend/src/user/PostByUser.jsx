            ...
            <>
                <div className="flex items-center justify-between mt-8 space-x-3">
                    ...
                    <div className="bg-white border border-2 border-gray-300 divide-gray-300 divide-x flex rounded-md shadow-sm dark:bg-gray-100">
                        <div className="bg-gray300 flex h-10 items-center justify-center  w-10" data-tippy-placement="top" data-tippy="" data-original-title="Grid view" onClick={() => setA(true)}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"  viewBox="0 0 16 16" className={"bi bi-border-all" + a ? "text-pink-600" : ""}>
                                <path d="M0 0h16v16H0V0zm1 1v6.5h6.5V1H1zm7.5 0v6.5H15V1H8.5zM15 8.5H8.5V15H15V8.5zM7.5 15V8.5H1V15h6.5z" />
                            </svg>
                        </div>
                        <div className="flex h-10 items-center justify-center w-10" data-tippy-placement="top" data-tippy="" data-original-title="List view" onClick={() => setA(false)}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className={"bi bi-card-list" + a ? "" : "text-pink-600"} viewBox="0 0 16 16">
                    ...
                    {posts.list?.length > 0
                        ? posts.list?.map((post, i) => (
                              <Link to={`/post/${post.id}`} key={i}>
                                  ...
                                  <div className={`bg-gray-200 max-w-full rounded-md relative overflow-hidden uk-transition-toggle ${a ? "lg:h-64 h-40" : "h-[40rem] w-full"}`} tabIndex={0}>
                                      {
                                          <img
                                              alt="post img"
                                              src={post.recipe.steps[0] ? post.recipe.steps[0].image : ""}
                                              onError={(i) => (i.target.src = `https://source.unsplash.com/random/?bakery,bake,1`)}
                                      ...
                                      <div className="absolute bg-black bg-opacity-40 bottom-0 flex h-full items-center justify-center space-x-5 text-lg text-white uk-transition-scale-up w-full">
                                          {post.likes.length}
                                          <ion-icon name="heart" className="mr-1 md hydrated" role="img" aria-label="heart" />
