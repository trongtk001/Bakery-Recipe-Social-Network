                ...
                <Link to={`/post/${post.id}`}>
                    <div className="flex flex-wrap h-[30rem] cursor-pointer">
                        ...
                        {post.recipe?.steps &&
                            post.recipe.steps?.map((item, index) => {
                                    ...
                                    return (
                                        <div key={index} className="w-full h-full" style={{ height: "30rem" }}>
                                            <img
                                                ...
                                                onError={(i) => (i.target.src = `https://source.unsplash.com/random/?bakery,bake,${item.image}`)}
                                            />
                                        </div>
                                    );
                                }
                                if (post.recipe.steps.length === 2) {
                                    return (
                                        <div key={index} className="w-full  border-2 border-white rounded-md overflow-hidden" style={{ height: "15rem" }}>
                        ...
                        {post.recipe.steps.length >= 3 ? (
                            <div
                                className=" relative h-[15rem] w-[50%] object-cover bg-gray-500 p-[70px] pl-[100px] rounded-md overflow-hidden border-white border-2"
                                style={{ backgroundImage: `url(https://source.unsplash.com/random/?bakery,bake,1)` }}
                            >
